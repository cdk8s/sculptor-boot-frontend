...
import TreeTable from '@/components/HehForm/TreeTable';
import { InputCron } from '@/components/Corn';
...
class HehForm extends Component<IProps, {}> {
  ...
  renderInput = (item: IInput) => {
    ...
    return (
      <Col style={item.style} span={24 / rowCols} key={item.id}>
        ...
        <Form.Item colon={false} label={newLabel} {...this.formItemLayout} {...item.formItemOptions}>
          {
            form.getFieldDecorator(item.id, {
              ...
              >
                {
                  item.options && item.options.map((child: any) => (
                    <Radio key={child.value || child.key || child.id} value={child.value}>{child.label}</Radio>
                  ))
                }
              </Radio.Group>,
            )
        ...
        <Form.Item colon={false} label={newLabel} {...this.formItemLayout} {...item.formItemOptions}>
          {
            form.getFieldDecorator(item.id, {
              ...
              <ChooseIcon attribute={item.attribute}/>,
            )
          }
        </Form.Item>
      </Col>
  ...
  render() {
    ...
    return (
      <Row>
        {
          // @ts-ignore
          formItems && formItems.map((item: IItem) => {
            switch (item.type) {
              case 'input':
              ...
              case 'treeTable':
                return this.renderTreeTable(item);
              case 'corn':
                return this.renderCron(item);
              case 'chooseIcon':
                return this.renderChooseIcon(item);
              case undefined:
                return false;
              default:
